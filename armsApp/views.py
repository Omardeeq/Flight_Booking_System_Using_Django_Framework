...
def context_data():
    context = {
        'page_name' : '',
        'page_title' : '',
        'system_name' : 'Flight Booking System',
        'topbar' : True,
        'footer' : True,
    }
...
def can_view_airport(view_func):
    @wraps(view_func)  # Use @wraps decorator here
    def wrapper(request, *args, **kwargs):
        if request.user.has_perm("armsApp.view_airport"):
            return view_func(request, *args, **kwargs)
        else:
            messages.warning(request, "You do not have permission to view airports.")
            return redirect('/home')  # Adjust the redirect URL as needed
    return wrapper


def can_view_flight(view_func):
    @wraps(view_func)  # Use @wraps decorator here
    def wrapper(request, *args, **kwargs):
        if request.user.has_perm("armsApp.view_flights"):
            return view_func(request, *args, **kwargs)
        else:
            messages.warning(request, "You do not have permission to view Flights.")
            return redirect('/home')  # Adjust the redirect URL as needed
    return wrapper

def can_view_reservation(view_func):
    @wraps(view_func)  # Use @wraps decorator here
    def wrapper(request, *args, **kwargs):
        if request.user.has_perm("armsApp.view_reservation"):
            return view_func(request, *args, **kwargs)
        else:
            messages.warning(request, "You do not have permission to view reservations.")
            return redirect('/home')  # Adjust the redirect URL as needed
    return wrapper
#add__permisions
def can_add_airline (view_func):
    def wrapper(request, *args, **kwargs):
        if request.user.has_perms('add_airlines'):
            return view_func(request, *args, **kwargs)
        else:
            messages.warning(request, "You do not have permission to add airline.")
            return redirect('/home') 
    return wrapper
# add airport permission
def can_add_airport (view_func):
    def wrapper(request, *args, **kwargs):
        if request.user.has_perms('add_airport'):
            return view_func(request, *args, **kwargs)
        else:
            messages.warning(request, "You do not have permission to add airport.")
            return redirect('/home') 
    return wrapper
#add flight
def can_add_airline (view_func):
...
def list_airline(request):
    user = request.user
    perm_code = "armsApp.view_airlines"
    has_perms = user.has_perm(perm_code)
